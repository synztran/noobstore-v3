...
const ModalPostProduct: React.FC<IProps> = ({ open = false, handleClose }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      productInfo: Yup.string().required("Thông tin sản phẩm là bắt buộc"),
      ...
      salePrice: Yup.number()
        ...
        .test(
          ...
          function (value) {
            return value === undefined || value <= this.parent.ogPrice;
          }
        ),
  ...
  const totalPrice = useMemo(() => {
    if (formik.values.salePrice !== "" && formik.values.salePrice !== "0") {
      return parseInt(formik.values.salePrice, 10) + totalServiceAddonPrice;
    }
  ...
  const syncImageToFormik = (files: { publicUrl: string; size: number }[]) => {
      ...
      <Modal open={open}>
        <Box
          className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[70vw] max-h-[90vh] bg-white shadow-lg px-8 pt-8 pb-4 rounded-lg ${
            formik.isSubmitting ? "overflow-hidden" : "overflow-y-auto"
          }`}
        >
          <Typography
            variant="h6"
          ...
          >
            ...
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <FormControl fullWidth>
                  ...
                  >
                    {postProductTypeOptions.map((option) => (
            ...
            {formik.values.priceType === EnumPostPriceType.ABSOLUTE ? (
              <Grid container spacing={2} className="">
                <Grid item xs={6}>
                  <TextField
                    ...
                    onBlur={formik.handleBlur}
                    error={
                      formik.touched.ogPrice && Boolean(formik.errors.ogPrice)
                    }
                    helperText={formik.touched.ogPrice && formik.errors.ogPrice}
                  />
                  {!formik.errors.ogPrice ? (
                    <span className="text-gray-500 text-xs mt-1 ml-4">
                      {formatCurrency(
                        parseInt(String(formik.values.ogPrice), 10) || 0
                      )}
                    </span>
                  ) : null}
                </Grid>
                <Grid item xs={6}>
                  ...
                  <span className="text-gray-500 text-xs mt-1 ml-4">
                    {formatCurrency(
                      parseInt(String(formik.values.salePrice), 10) || 0
                    )}
                  </span>
                </Grid>
              </Grid>
            ...
            <UploadImage
              ...
              errorMessage={(formik.errors.images as string) || ""}
            ...
            <Box component={"fieldset"} className="border p-4 rounded-lg">
              ...
              <div className="flex flex-col pl-4 gap-1">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500 text-sm">Giá khuyến mãi</span>
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500 text-sm">Phí nền tảng</span>
                  <span className="text-gray-500 text-sm">
                    {/* {formatCurrency(0)} */}
                    Bạn còn được <strong>3 lượt miễn phí</strong> ở tháng này
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-500 text-base font-bold">
                    {formatCurrency(totalPrice + totalServiceAddonPrice)}
                  </span>
                </div>
              </div>
            </Box>
...
const PolicyAndCondition = () => {
  return (
    <div className="flex flex-col gap-2">
      <p className="text-sm">
        Tôi đồng ý với{" "}
        <span className="text-blue-500 cursor-pointer">Điều khoản dịch vụ</span>{" "}
        và{" "}
          ...
          <div className="grid grid-cols-[32px_auto_120px] gap-2">
            <Checkbox
              ...
              onChange={(event) => {
                ...
                const currentServices = formik.values.addonServices;
                const serviceAddonPrice = checkUsedServiceAddonPrice({
                  isPercent: service.isPercent,
                  productPrice:
                    formik.values.salePrice || formik.values.ogPrice,
                ...
                if (isChecked) {
                  ...
                  formik.setFieldValue(
                    ...
                    currentServices.filter(
                      (item: { value: string; price: number }) =>
            ...
            <div className="flex gap-2">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500">{service.subName}</div>
              </div>
            </div>
