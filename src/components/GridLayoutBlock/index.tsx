...
import { GROUP_IMAGE_1, GROUP_IMAGE_10, GROUP_IMAGE_11, GROUP_IMAGE_12, GROUP_IMAGE_13, GROUP_IMAGE_14, GROUP_IMAGE_2, GROUP_IMAGE_3, GROUP_IMAGE_4, GROUP_IMAGE_5, GROUP_IMAGE_6, GROUP_IMAGE_7, GROUP_IMAGE_8, GROUP_IMAGE_9, LOGO_STORE } from '@/constants/Images'
...
const GridLayoutBlock: React.FC<IProps> = ({ images }) => {
  return (
    <div className="w-full">
      <div className="mb-4">
        <span className="font-bold text-2xl text-black">Được chia sẽ trên nền tảng Instagram.</span>
        <span className="text-gray-500 text-base font-normal ml-2">Sức sáng tạo là không giới hạn.</span>
      </div>
      <div
        ...
        {communityImages.map((item, idx) => (
          ...
          >
            <div className="relative w-full h-full min-h-[220px]">
              <Image src={item.src} alt={`Community build by ${item.owner}`} fill className="transition-transform duration-300 group-hover:scale-105 object-cover" priority={idx < 3} />
              ...
              <div className="absolute bottom-0 left-0 w-full flex items-center justify-between px-3 py-2 bg-gradient-to-t from-black/70 to-transparent backdrop-blur-[5px]">
                <span className="text-white font-semibold text-sm flex items-center gap-1">@{item.owner}</span>
                {item.details ? (
                  <Tooltip placement="left-end" title={<DetailTooltip detail={item.details || {}} />} arrow>
      ...
      <div className="mt-8 flex flex-col items-center justify-center gap-4">
        <div className="flex items-center gap-4">
          <div className="w-24 h-24 bg-gray-200 rounded-full relative overflow-hidden border border-gray-600  ">
            <Image src={LOGO_STORE} alt="Noob Assembly" fill className="p-0.5 rounded-full object-cover" />
          </div>
          <div className="flex flex-col">
            <span className="text-black text-2xl font-bold">NoobStore</span>
          ...
          <a href="https://www.facebook.com/noobassembly" target="_blank" rel="noopener noreferrer">
            <button className="bg-blue-600 text-white px-6 py-2 rounded-md hover:scale-105 transition-all duration-300 text-lg">Follow us</button>
          </a>
        </div>

        <div className="text-black text-xl max-w-lg text-center">
          Chia sẻ ảnh và video trên instagram với hashtag <strong className="text-blue-600 text-xl">#noobstore</strong> hoặc <strong className="text-blue-600 text-xl">@noobstore</strong> để có cơ hội chia sẻ với mọi người ở đây nhé ♥️
        </div>
      </div>
    </div>
  )
