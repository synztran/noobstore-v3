...
const RaffleV2Countdown: React.FC<RaffleCountDownProps> = ({ startDate, endDate, raffleStatus, className = '' }) => {
  ...
  const AnimatedDigit = ({ value }: { value: string | number }) => (
    <AnimatePresence mode="wait" initial={false}>
      <motion.span key={value} variants={digitVariants as any} initial="initial" animate="animate" exit="exit" className={`inline-block text-lg ${mappingRaffleStatusLabel[raffleStatus]?.iconColor}`} style={{ minWidth: '2ch' }}>
        {value}
      </motion.span>
    </AnimatePresence>
  )
  ...
  return (
    <div className={`border border-gray-200 bg-gray-50 rounded-md p-2 flex items-center justify-between gap-2 ${fancyBg} ${className}`}>
      <div className={`clear-both relative uppercase font-semibold text-sm inline-flex gap-1 items-end`}>
        <Timer size={20} className={`${mappingRaffleStatusLabel[raffleStatus]?.iconColor} scale-110`} />
        <div className="leading-4">{mappingRaffleStatusLabel[raffleStatus]?.label}</div>
      </div>
      {mode !== EnumRaffleStatus.ENDED ? (
        ...
        <AnimatePresence mode="wait" initial={false}>
          <motion.span key="raffle-ended" variants={textVariants} initial="initial" animate="animate" exit="exit" className="text-xl font-semibold" style={{ color: '#111' }}>
            {/* Raffle đã kết thúc. */}
            00:00:00
          </motion.span>
        </AnimatePresence>
