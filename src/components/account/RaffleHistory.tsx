...
const RaffleHistory: React.FC = () => {
  ...
  const mapRaffleProductWin = useMemo(() => {
    return (
      raffleHistory?.find((raffle) => raffle.raffleId === selected?.raffleId)
        ?.productSelections || []
    ).reduce(
      (acc, product) => {
          ...
          >
            ...
            <div className="w-full flex flex-col gap-2 justify-between">
              <div className="relative">
                <div className="flex justify-between items-start gap-2">
                  <span className="font-bold line-clamp-2">
                    {entry.raffleInfo.title || "Raffle không xác định"}
                  </span>
                </div>
              ...
              <div className="text-sm text-gray-800">
                Ngày tham gia: {new Date(entry.joinedAt).toLocaleDateString()}
              </div>
              {entry.isWinner ? (
                <div
                  // onClick={(e) => {
                  ...
                  // }}
                  className={`${
                    mappingEnumRafflePaymentStatus[entry.paymentStatus]?.bgColor
                  } ${
                    mappingEnumRafflePaymentStatus[entry.paymentStatus]?.color
                  } rounded-md max-w-max text-sm font-semibold text-center px-2 py-1 ml-auto ${
                    entry.isWinner &&
                    entry.paymentStatus !== EnumRafflePaymentStatus.PAID
                      ? "hover:scale-105 cursor-pointer shadow-md transition-all duration-200 z-10"
                      : ""
                  }`}
                >
                  {mappingEnumRafflePaymentStatus[entry.paymentStatus]?.label}
                </div>
              ) : (
                <div
                  className={`${
                    mappingRaffleStatusLabel[entry.raffleInfo.status]?.color
                  } ${
                    mappingRaffleStatusLabel[entry.raffleInfo.status]?.bgColor
                  } rounded-md font-semibold px-2 py-1 text-sm max-w-max ml-auto overflow-hidden flex items-center gap-1`}
                >
                  {mappingRaffleStatusLabel[entry.raffleInfo.status]?.icon ? (
                    <Image
                      src={
                        mappingRaffleStatusLabel[entry.raffleInfo.status]?.icon!
                      }
                  ...
                  {mappingRaffleStatusLabel[entry.raffleInfo.status]?.label}
                </div>
              )}
            </div>
          </div>
