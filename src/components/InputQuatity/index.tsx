...
const InputQuantity = ({ triggerResetQuantity, quantity, productId, className, maxQuantity, productOptionId, isBuyGroup }: Props) => {
  const { updateQuantity } = useStoreProductDetailAction()
  const [currentQuantity, setCurrentQuantity] = useState(quantity || 1)
  const { data: cart } = useCartQuery()
  const { mutate: updateQuantityMutation, isPending } = useCartQuantityMutation()
  ...
  const handleDecreaseQuantity = async (e: React.MouseEvent<HTMLElement>) => {
    ...
    if (isBuyGroup) {
      NotifyUtils.error('Sản phẩm này mua theo kit, không thể thay đổi số lượng đã chọn')
      return
    }
  ...
  const handleDecreaseQuantityCart = useCallback(async () => {
    if (currentQuantity <= 1) return
    if (isBuyGroup) {
      NotifyUtils.error('Sản phẩm này mua theo kit, không thể thay đổi số lượng đã chọn')
      return
  ...
  }, [currentQuantity, productId, user?.cartId, productOptionId, updateQuantityMutation])
  ...
  }, [quantity, productId, user?.cartId, productOptionId, updateQuantityMutation])
  ...
  const handleOnChange = async (quantity: number, notValidCallback?: () => void) => {
    if (quantity <= 0) return
    if (!quantity) return
    if (maxQuantity && quantity > maxQuantity) {
      NotifyUtils.error('Số lượng sản phẩm không được lớn hơn số lượng tồn kho')
  ...
  return (
    <div className={classNames('flex align-middle items-centers border border-gray-400 rounded-md max-w-max', className ?? '')}>
      <MinusButton
      ...
      />
      <ProductQuantity quantity={currentQuantity} handleOnChange={handleOnChange} />
      <AddMoreButton
...
const ProductQuantity = memo(({ quantity, handleOnChange }: { quantity: number; handleOnChange: (quantity: number, notValidCallback?: () => void) => void }) => {
