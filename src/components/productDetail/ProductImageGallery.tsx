...
const ProductImageGallery = ({ images, status, productName }: ProductImageGalleryProps) => {
  ...
  return (
    <div className="w-full lg:w-3/5 border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-6 flex flex-col justify-center">
      {/* Main Image */}
      <div className="relative aspect-4/3 rounded-lg overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm mb-4">
        ...
        {status && statusConfig[status] && (
          <div className="absolute top-4 left-4 z-10">
            <span className={`text-xs font-bold px-3 py-1.5 rounded uppercase tracking-wider shadow-sm ${statusConfig[status]?.className}`}>{statusConfig[status]?.label}</span>
          </div>
        )}
      </div>
