...
export const RaffleSpecifications = ({ raffle }: RaffleSpecificationsProps) => {
  ...
  return (
    <section className="space-y-4">
      <h3 className="text-xl md:text-2xl font-bold text-slate-900 flex items-center gap-2">Specifications</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {specs.map((spec, idx) => (
          <div key={idx} className="flex items-center p-4 bg-slate-50 border border-slate-200 rounded-xl hover:border-amber-400/40 transition-all group shadow-sm">
            ...
            <div>
              <p className="text-sm uppercase tracking-widest text-slate-500 font-bold mb-1">{spec.label}</p>
