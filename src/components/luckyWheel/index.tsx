...
const LuckyWheelContainer = () => {
  ...
  const renderBorder = () => {
    const canvas = document.getElementById("border-wheel") as HTMLCanvasElement;
    if (canvas) {
  ...
  const renderCenterWheel = () => {
    const canvas = document.getElementById("center-wheel") as HTMLCanvasElement;
    if (canvas) {
      const ctx = canvas.getContext("2d");
      const cw = canvas.width;
      const ch = canvas.height;
      ...
      const calcPosition = (
        ...
        if (image.naturalWidth === image.naturalHeight) {
          if (rewardCount >= 10) {
            ...
            return {
              x: (265 / 2 + 1) * Math.cos(sweep * i) + imageItemW + 25,
              y: (265 / 2 + 1) * Math.sin(sweep * i) + imageItemH + 25,
            };
          }
          // in case less than 10 items, w & h of prize is bigger, So need adjust a base  and eror rate
          return {
            x: (250 / 2 + 3) * Math.cos(sweep * i) + imageItemW + 10,
            y: (250 / 2 + 3) * Math.sin(sweep * i) + imageItemH + 10,
          };
        }
  ...
  const spin = async () => {
    ...
    if (resSpin && resSpin.data) {
      ...
      if (rewardIdxMap.get(itemCode)) {
        idx =
          (spinnerInfo.top as number) - (rewardIdxMap.get(itemCode) ?? index); // idx will start at 0
      } else {
      ...
      setTimeout(() => {
        ...
        const formatData: IReward = {
          ...
          createdTime: resSpin.data[0].createdTime || new Date().toISOString(),
          ...
          maxQuantityPerCustomer: resSpin.data[0].maxQuantityPerCustomer,
  ...
  const activeButtonSpin = () => {
    if (btnSpinHeadRef && btnSpinHeadRef.current) {
      btnSpinHeadRef?.current?.classList.add(styles.btnSpin_active as string);
    }
  };
      ...
      if (true) {
        ...
        const newSpinnerIdxMap = new Map(
          items
            ?.filter((item: any) => item.isActive)
            .sort((a: any, b: any) => a.index - b.index)
            ?.map((child: any, idx: any) => [child.itemCode, idx + 1])
        );
  ...
  useEffect(() => {
    const intervalFreeSpin = setInterval(() => {
      ...
      if (
        currentDate.getTime() >=
        new Date(spinnerInfoRef?.current?.nextFreeTime as string).getTime()
      ) {
            ...
            >
              <div
                ...
                style={{
                  ...
                  left: item.itemX,
                  rotate: `${
                    -90 + item.i * (360 / spinnerInfo.rewards?.length)
                  }deg`,
                  zIndex: 1,
  ...
  return (
    <div id="spinnerWrapper" className={styles.spinnerReward_wrapper}>
      ...
      >
        {/* title */}
        <Grid item sm={3} md={3} className={styles.sectionWheelTitle_container}>
          <div
        ...
        <Grid item sm={5} md={5}>
          ...
          >
            <Box className={styles.drawCanvas_container}>
              <div id="spin_1" className={styles.luckyWheel_central} />
              ...
              {!spinnerInfo.spinning ? (
                <TempPrizeElement listItem={mapRewardPosition} />
              ) : null}
            </Box>
            <ButtonSpin
              spin={spin}
            ...
            >
              <Grid container className={styles.boardTab}>
                <Grid
                  ...
                  className={clsx(
                    styles.missionTab,
                    historyBoardTab === ENUM_HISTORY_BOARD_TAB.MISSION &&
                      styles.active
                  )}
                  onClick={() =>
                    setHistoryBoardTab(ENUM_HISTORY_BOARD_TAB.MISSION)
                ...
                <Grid
                  ...
                  className={clsx(
                    ...
                    historyBoardTab === ENUM_HISTORY_BOARD_TAB.HISTORY_REWARD &&
                      styles.active
                  )}
                  onClick={() =>
                    setHistoryBoardTab(ENUM_HISTORY_BOARD_TAB.HISTORY_REWARD)
                ...
                <Grid
                  ...
                  className={clsx(
                    styles.guildLine,
                    historyBoardTab === ENUM_HISTORY_BOARD_TAB.GUILDLINE &&
                      styles.active
                  )}
                  onClick={() =>
                    setHistoryBoardTab(ENUM_HISTORY_BOARD_TAB.GUILDLINE)
              ...
              {historyBoardTab === ENUM_HISTORY_BOARD_TAB.HISTORY_REWARD &&
                ...
                spinnerLog.total > 0 && (
                  <Box className={clsx(styles.boardContainer)}>
                    ...
                    <Box
                      className={styles.boardContainer_childList}
                    ...
                    >
                      {spinnerLog?.data?.map((item) => (
                        <Grid
                          key={item.itemCode}
                          container
                          className={styles.boardItem_container}
                        >
                          <Grid
                            item
                            md={3}
                            className={styles.boardItem_imageWrapper}
                          >
                            <NextImage
                              // src={getLinkProxyCDN(
                              // 	item.itemImage ??
                          ...
                          <Grid item md={9}>
                            <Typography
                              variant="body2"
                              className={styles.boardItem_itemName}
                            >
                              {" "}
                              {item?.message
                                ?.replace("Nhận được", "")
                                .replace("từ vòng quay may mắn", "")}
                            </Typography>
                            <span className={styles.boardItem_created}>
                              {/* {format(
																new Date(
																	item.createdTime
																),
              ...
              {historyBoardTab === ENUM_HISTORY_BOARD_TAB.GUILDLINE && (
                <Box className={clsx(styles.boardGuildLine)}>
                  <Box className={styles.historyBoard_guildContainer}>
                    <div
                      className={styles.historyBoard_guildLineText}
                    ...
                    {spinnerMission
                      ...
                      ?.map((item) => (
                        <Box
                          ...
                          className={styles.boardMissionItem_container}
                        >
                          <Box>
                            <Typography
                              variant="body2"
                              className={styles.boardMissionItem_itemName}
                            >
                              {" "}
                              {item.missionName}
                            </Typography>
                            ...
                            <div
                              ...
                              dangerouslySetInnerHTML={{
                                __html: item?.condition?.description || "",
                              }}
                            />

                            <Box
                              className={clsx(
                                item?.processInfos?.length > 0 &&
                                      ...
                                      >
                                        ...
                                        >
                                          <BorderLinearProgress
                                            variant="determinate"
                                            value={
                                              process.value / process.target > 1
                                                ? 100
                                                : (process.value /
                                                    process.target) *
                                                  100
                          ...
                          {item?.processInfos &&
                          item?.processInfos.filter((item) => item.isCompleted)
                            ?.length !== item?.processInfos?.length ? (
                            <Box
                              style={{
                                marginLeft: "auto",
                              }}
                            ...
                            >
                              <CheckCircleOutlineIcon /> Đã hoàn thành
                            </Box>
                          )}
                          {item?.isBlocked &&
      ...
      <Box>
        ...
        {type === "reward" ? (
          <Typography className={styles.popupWarning_subTitle} variant="body2">
            Bạn đã sở hữu toàn bộ phần thưởng! <br /> Mong bạn sẽ tiếp tục ủng
            hộ các chương trình tiếp theo đến từ thuocsi.vn
          </Typography>
        ) : (
          <Typography className={styles.popupWarning_subTitle} variant="body2">
            {subTitle}
...
const PopupGotPrize = ({ open, handleClose, reward }: IPopupGotPrize) => {
  ...
  const textHightLight =
    highLightSplitText(reward?.message as string, "<strong>", "</strong>") ??
    reward?.message;
    ...
    <Modal open={open}>
      <Box sx={style} style={{ outline: 0 }}>
        ...
        <Box>
          ...
          <Typography className={styles.popupGotPrize_subTitle} variant="body2">
            <div
              style={{ fontSize: "16px" }}
              dangerouslySetInnerHTML={{
                __html:
