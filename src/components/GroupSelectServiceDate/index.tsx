...
import useServices, { IServicePlan, useServiceAction } from '@/zustand/useServices'
...
const GroupSelectServiceDate = (props: IProps) => {
  ...
  const currentSelection = selectedPlan || selectedOpt || (planOptions?.[0] as IServicePlan)
    ...
    return (
      <div className="w-full grid grid-cols-2 gap-2">
        {Array.from({ length: 2 }).map((_, idx) => (
          <div key={idx} className="h-40 bg-white rounded-md p-4 flex flex-col justify-between shadow relative">
            <div>
  ...
  return (
    <div className={`w-full relative`} id="service-plan-selection">
      ...
      {errorMessages?.selectedPlan && (
        <div className="mb-2">
          <ErrorMessage message={errorMessages.selectedPlan} type="error" autoHide />
        </div>
      )}
      <FormControl className={`!grid grid-cols-${planOptions?.length} gap-2 w-full`}>
        {planOptions?.map((option, index) => (
          <RadioGroup key={index} aria-labelledby="demo-radio-buttons-group-label" defaultValue={planOptions?.[0]?.planId} defaultChecked={option.planId === currentSelection?.planId} name="radio-buttons-group">
            <Wrapper option={option as unknown as IServicePlan} key={index}>
              <FormControlLabel
                checked={option.planId === currentSelection?.planId}
                value={option.planId}
                control={<Radio className="p-0" />}
                label=""
                classes={{ root: 'mr-0' }}
                onChange={() => {
                  selectPlan(option as unknown as IServicePlan)
                }}
              />
            </Wrapper>
          </RadioGroup>
...
const Wrapper = ({ children, option, onClick }: { children: React.ReactNode; option: IServicePlan; onClick?: () => void }) => {
  ...
  return (
    <div className={`w-full min-h-[160px] rounded-xl p-4 border-2 shadow-lg bg-gray-100 flex flex-col justify-between cursor-pointer hover:border-blue-500 hover:shadow-lg ${option.planId === currentSelection?.planId ? 'border-blue-700' : ''}`} onClick={() => selectPlan(option)}>
      ...
      <div className="flex justify-between items-end mt-auto">
        ...
        <span className="font-bold text-[24px]">{option.price > 0 ? formatCurrency(option.price) : ''}</span>
      </div>
    </div>
  )
}
