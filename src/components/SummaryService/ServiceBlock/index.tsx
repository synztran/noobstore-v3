      ...
      switch (name) {
        case "keyboard": {
          ...
          if (value && validKeys.includes(key)) {
            switch (key) {
              ...
              case "pcb":
                const pcbLabel =
                  value === "SOLDER" ? "Mạch hàn" : "Mạch hot-swap";
        ...
        case "switch": {
          ...
          if (value && validKeys.includes(key)) {
            switch (key) {
              case "status":
                const statusLabel = value === "NEW" ? "Mới" : "Đã qua xử lý";
          ...
          if (value && validKeys.includes(key)) {
            switch (key) {
              ...
              case "type":
                ...
                break;
              case "status":
                const statusLabel = value === "NEW" ? "Mới" : "Đã qua sử lý";
                infoList.push({ key, value: statusLabel });
                break;
              case "totalPack":
      ...
      <div className="flex flex-col w-full">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            {totalPrice > 0 ? (
              <strong className="text-base">{totalService} Dịch vụ</strong>
            ) : (
              <CircularProgress size={20} />
            )}
            {totalService > 0 ? (
              <BtnCollapse isToggled={isCollapsed} toggle={setIsCollapsed} />
            ) : null}
          </div>
          {totalPrice > 0 ? (
            <div className="text-base">{formatCurrency(totalPrice)}</div>
          ) : null}
        </div>
        {totalService > 0 ? (
          <SummaryServiceCollapse isCollapse={isCollapsed}>
            <div className="flex flex-col gap-2 pr-2 mt-2">
                    ...
                    <div className="flex flex-col w-full">
                      <div className="flex flex-col gap-1">
                        <span className="text-base font-medium min-h-[24px] capitalize">
                          {item.keyboard.name || <CircularProgress size={20} />}
                        </span>
                        <div className="flex flex-col gap-2">
                          <DisplayInfo
                            label="Phím:"
                            values={listInfo("keyboard", index)}
                          />
                          <DisplayInfo
                            label="Switch:"
                            values={listInfo("switch", index)}
                          />
                          <DisplayInfo
                            label="Stabilizer:"
                            values={listInfo("stabilizer", index)}
                          />
                        </div>
                      </div>
                      ...
                      {Object.keys(item.services).length ? (
                        <div className="flex flex-col gap-2">
                          <DisplayServiceAndPrice
                            key={item.id}
                            service={item.services?.keyboard}
                            parentName="keyboard"
                            switchQuantity={item.switch.quantity}
                            packs={item.stabilizer.packs}
                            wires={item.stabilizer.wires}
                          />
                          <DisplayServiceAndPrice
                            key={item.id}
                            service={item.services?.switch}
                            parentName="switch"
                            switchQuantity={item.switch.quantity}
                          />
                          <DisplayServiceAndPrice
                            key={item.id}
                            service={item.services?.stabilizer}
                          ...
                          <div className="flex gap-2 text-right justify-end items-center border-t border-gray-300 max-w-max ml-auto pt-1">
                            ...
                            <span className="text-right text-base">
                              {formatCurrency(
                                getPriceKeyboardServiceById(item.id)
                              )}
                            </span>
                          </div>
                        </div>
              ...
              {switchItems.map((sw, idx) => (
                <div className="flex flex-col" key={`${sw.name}-${idx}`}>
                  <div className="flex items-start gap-1">
                    <Image
                    ...
                    <div className="flex flex-col w-full">
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex gap-2 text-right justify-end items-center border-t border-gray-300 max-w-max ml-auto pt-1">
                          ...
                          <span className="text-right text-base">
                            {formatCurrency(
                              getPriceServiceById(sw.id, "switch")
                            )}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col w-full">
                      <div className="flex flex-col">
                        ...
                        <div className="flex gap-1">
                          ...
                          <span className="border border-gray-700 rounded-md p-1 leading-[1] text-sm text-gray-700">
                            {stabilizer.wires.length} thanh wire
                          </span>
                        </div>
                      </div>
                      <Divider className="!mt-1.5 !mb-1" />
                      <div className="flex flex-col px-2">
                        {Object.entries(stabilizer?.services || {}).map(
                            ...
                            if (
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <span className="text-base">
                                    {formatCurrency(
                                      service.price * stabilizer.wires.length
                                    )}
                                  </span>
                                </div>
                              );
  ...
  return (
    <div className="flex flex-col w-full gap-2">
      {Object.entries(service).map(([name, serviceData], index) => {
        ...
        const displayServiceName = mappingServiceName[parentName][name] || name;
        ...
        return (
          ...
          >
            ...
            <span className="text-sm">{formatCurrency(formattedPrice)}</span>
          </div>
        );
      })}
    </div>
