...
const MusicPlayList: React.FC<IProps> = ({ open, onClose, audioRef }) => {
	...
	useEffect(() => {
		const loadAudioMetadata = async (
			...
			return new Promise<typeof song & { duration: string }>(
				(resolve) => {
					audio.addEventListener("loadedmetadata", () => {
						const stringDuration = convertToPlayerTimer(
							audio.duration
						);
						resolve({
							...song,
	...
	return (
		<Drawer
			...
			disableScrollLock
			classes={{
				paper: "max-w-[max(480px,30vw)] ml-auto border border-black rounded-lg shadow-lg bottom-[63px] right-[10%]",
			}}
