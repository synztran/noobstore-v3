...
const DonationBlock = (props: { service: IResponseBackendServiceBooking }) => {
  ...
  const selectedDonation = useMemo(() => {
    if (donation) {
      return (
        donationOptions.find((option) => option.value === donation?.value) ||
        null
      );
    }
    ...
    <div className="flex flex-col gap-2">
      <div className="flex flex-col gap-1">
        ...
        <small className="text-sm">
          (*) Shop sẽ trích 50% tiền ủng hộ để đóng góp vào quỷ hỗ trợ trẻ em -{" "}
          <a
            href="#"
            target="_blank"
        ...
        {donationOptions?.map((option, index) => (
          <div className="relative" key={option.id}>
            ...
            >
              ...
              >
                <FormControlLabel
                  checked={option.percentage === selectedDonation?.percentage}
      ...
      {selectedDonation ? (
        <div className="relative mt-4">
          ...
          <textarea
            ...
            disabled={service?.paymentStatus === EnumPaymentStatus.PAID}
          />
        </div>
      ) : null}
    </div>
