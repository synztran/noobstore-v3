...
const DialogLogin = ({ isOpen, toggleOpen }: Props) => {
  ...
  const [formType, setFormType] = useState<ENUM_FORM_TYPE>(
    ENUM_FORM_TYPE.LOGIN
  );

  // schema for formik
  const LoginSchema = Yup.object().shape({
    email: Yup.string()
      .email("Có vẻ sai định dạng rồi")
      .required("Bạn chưa nhập email"),
    password: Yup.string()
      .min(7, "Mật khẩu phải dài hơn 6 kí tự")
  ...
  const SigupSchema = Yup.object().shape({
    email: Yup.string()
      ...
      .required("Bạn chưa nhập email ạ"),
    password: Yup.string()
      .min(7, "Mật khẩu phải dài hơn 6   kí tự")
      .required("Bạn chưa nhập mật khẩu ạ"),
    confirmPassword: Yup.string()
      .oneOf([Yup.ref("password"), undefined], "Mật khẩu xác nhận không khớp")
      .required("Vui lòng xác nhận mật khẩu"),
    phoneNumber: Yup.string()
      .matches(vietnamesePhoneNumberRegex, "Số điện thoại không hợp lệ")
      .required("Bạn chưa nhập số điện thoại ạ"),
    // dob: Yup
    // .date()
    ...
    <Dialog
      ...
      PaperProps={{
        style: {
          ...
          maxWidth: formType === ENUM_FORM_TYPE.LOGIN ? "460px" : "650px",
          borderRadius: "8px",
        },
      }}
    >
        ...
        <div>
          <DialogTitle className="flex justify-between items-center">
            <span
              ...
              aria-label="123124124"
              aria-braillelabel="qweiquwoieu"
            >
              Chào mừng trở lại
            </span>
            <IconButton
                ...
                <Form className="mx-auto flex flex-col max-w-3xl gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="relative">
                      ...
                      <div
                        className="absolute top-0 right-4 w-6 h-6 translate-y-1/2 z-10 cursor-pointer"
                  ...
                  <Box width="100%" textAlign="center">
                    <button
                      className={classNames(
                        `w-2/3 px-4 py-2 rounded-md bg-black text-white hover:bg-black hover:opacity-80`,
                        onProcessing ? `pointer-events-none opacity-80` : ""
                      )}
                      type="submit"
                      aria-label="đóng"
                      aria-braillelabel="nút đóng"
                    >
                  ...
                  >
                    <span className="text-gray-600">
                      ...
                      <span
                        className="uppercase underline font-bold cursor-pointer"
                        onClick={() => setFormType(ENUM_FORM_TYPE.REGISTER)}
                      >
                        Đăng ký
                      </span>
                    </span>
                ...
                <Form className="mx-auto flex flex-col max-w-3xl gap-4">
                  ...
                  >
                    <span className="text-gray-600">
                      <span
                        className="uppercase underline font-bold cursor-pointer"
                        onClick={() => setFormType(ENUM_FORM_TYPE.LOGIN)}
                      >
                        Đã có tài khoản?
                      </span>
                    </span>
...
const NumericInput = ({ field, form, ...props }: { field: any; form: any }) => {
  ...
  return (
    <div>
      <input {...field} {...props} onKeyPress={handleKeyPress} />
