            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="w-20 h-20 bg-gray-200 animate-pulse rounded-full"
                  ></div>
                ))}
              </div>
            </div>
              ...
              <div className="w-full h-[34rem] relative">
                <AnimatePresence mode="wait" initial={false}>
                  <motion.div
                    ...
                    onDragEnd={(_, info) => {
                      const threshold = 60;
                      const length = raffleData?.productOptions?.length ?? 0;
                      if (!length) return;

                      // Slide left -> next image
                      if (info.offset.x < -threshold) {
                        handleImageChange((currentImageIndex + 1) % length);
                        return;
                      }
                  ...
                  >
                    <Image
                      src={
                        raffleData?.productOptions?.[currentImageIndex]
                          ?.thumbnail?.path || ""
                      }
                      alt={
                        raffleData?.productOptions?.[currentImageIndex]
          ...
          <div className="w-full max-h-max h-full rounded-xl shadow-md bg-white p-4 flex gap-4">
            <div className="flex items-center gap-3 mt-2 w-5/12">
              <div className="min-w-[80px] h-20 bg-gray-200 rounded-full relative overflow-hidden border border-gray-600">
                <Image
                  src={raffleData?.makerInfo?.logo?.path || NEW_MISSING_IMAGE}
                  alt={raffleData?.makerInfo?.brandName || "maker Avatar"}
                  fill
                  objectFit="cover"
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-1">
                  ...
                  {raffleData?.makerInfo?.verificationStatus ===
            ...
            >
              Maker cung cấp những sản phẩm thủ công với độ chi tiết đên từ các
              nguyên liệu tốt nhất. Mỗi sản phẩm đều được tạo ra với tâm huyết
              và sự tận tâm, mang đến trải nghiệm độc đáo và giá trị cao cho
              người sử dụng.
            </div>
          </div>
        </div>
        ...
        <div className="col-span-5 bg-white rounded-xl shadow p-4 flex flex-col gap-2 h-full space-y-2">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <RaffleStatus
                mode={raffleData.status ?? statusRaffleBasingTime}
              />
            </div>
            {/* <RaffleCountDown
          ...
          <div className="relative">
            <div className="font-semibold mb-1 flex items-center justify-between overflow-hidden">
              <span className="min-w-[90px] text-lg">Sản phẩm</span>
          ...
          <div className="relative">
            <div className="font-semibold mb-1 flex items-center justify-between overflow-hidden">
              <span className="min-w-[90px] text-lg">Thông tin</span>
              <Divider className="border-gray-400 w-[80%]" />
            </div>
            <div
            ...
            >
              {raffleData.features.map((feature, idx) => (
                <div key={idx} className="flex items-center gap-1">
                  <Dot size={24} strokeWidth={2} className="scale-150" />
          ...
          <div className="relative">
            <div className="font-semibold mb-1 flex items-center justify-between overflow-hidden">
              <span className="min-w-[90px] text-lg">Giao hàng</span>
              <Divider className="border-gray-400 w-[80%]" />
            </div>
            <div
            ...
            >
              ...
              <span className="font-semibold">
                {formatDistanceToNow(new Date(raffleData?.endAt || ""), {
                  addSuffix: true,
                })}{" "}
                ~{" "}
                <span className="font-semibold">
