...
const RaffleCountDown: React.FC<RaffleCountDownProps> = ({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const { mode: m, diff } = DateUtils.getTimeLeft(start, end, nowDate);
      ...
      {mode !== EnumRaffleStatus.ENDED ? (
        <div className="flex gap-1 text-[20px] font-mono font-bold items-start">
          <div className="flex flex-col items-center min-w-[44px]">
            ...
            <span className="text-base font-medium opacity-80">Ngày</span>
          </div>
          <div className="text-2xl h-[30px] relative bottom-0.5">:</div>
          <div className="flex flex-col items-center min-w-[44px]">
            <AnimatedDigit value={pad(timeLeft.hours)} />
            <span className="text-base font-medium opacity-80">Giờ</span>
          </div>
          <div className="text-2xl h-[30px] relative bottom-0.5">:</div>
          <div className="flex flex-col items-center min-w-[44px]">
            <AnimatedDigit value={pad(timeLeft.minutes)} />
            <span className="text-base font-medium opacity-80">Phút</span>
          </div>
          <div className="text-2xl h-[30px] relative bottom-0.5">:</div>
          <div className="flex flex-col items-center">
            <AnimatedDigit value={pad(timeLeft.seconds)} />
            <span className="text-base font-medium opacity-80">Giây</span>
          </div>
        </div>
      ) : (
        <AnimatePresence mode="wait" initial={false}>
