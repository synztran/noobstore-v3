...
const StepperRaffleFooter = ({
  ...
  const handleSubmitRaffle = async () => {
    const payload: IRequestRaffleJoin = {
      ...
      name:
        raffleSubmitForm?.name || user?.firstName + " " + user?.lastName || "",
      phone: raffleSubmitForm?.phone || user?.phoneNumber || "",
      shipping: raffleSubmitForm?.shipping as IRequestRaffleJoin["shipping"],
    ...
    if (resp.status === "OK") {
      ...
      NotifyUtils.error(ERROR_MESSAGES[resp.errorCode || ""] || resp.message);
  ...
  const renderNextButton = () => {
    ...
    const isSelectionStep = currentStep?.props.stepValue === "productSelection";
    const isSecretKeyStep = currentStep?.props.stepValue === "secretKey";
    const isInformationStep = currentStep?.props.stepValue === "information";
    ...
    if (isSubmitStep) {
      return (
        ...
        >
          {isSubmitting ? <CircularProgress size={22} /> : "Hoàn tất đăng ký"}
        </Button>
      );
    }
