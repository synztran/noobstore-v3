...
const StabilizerSelection: React.FC<IProps> = ({
  ...
  const handleAddPack = () => {
    ...
    if (selectedPack.type === "pack" && stabilizerSelected.totalPack >= maxPack)
      return;
  ...
  const handleAddOther = () => {
    ...
    if (stabilizerSelected.totalWire + otherSelection.quantity > maxTotalWire)
      return;
    ...
    const updater = exists
      ...
      : {
          ...
          name:
            INDIVIDUAL_OPTIONS.find((opt) => opt.value === otherSelection.value)
    ...
    <div className={`flex flex-col gap-4 ${className || ""}`}>
      <>
        <label className="font-semibold block">
          ...
          <span className="text-sm font-normal text-gray-600">
            Tối đa {maxPack} set hoặc tổng số stabilizer ≤ {maxTotalWire}
          </span>
        </label>
          ...
          {PACK_OPTIONS.map((pack) => (
            ...
            >
              ...
              <span className="text-sm font-bold">
                {pack.label} &nbsp;
                {pack.detail ? (
                  <span className="text-xs text-gray-900">({pack.detail})</span>
                ) : null}
              </span>
            </button>
          ))}
      ...
      <InputWrapperLegend label="Stabilizer đã chọn">
        ...
        !stabilizerSelected.wires.length ? (
          <div className="text-gray-500">Chưa chọn stabilizer nào.</div>
        ) : (
          <div className="flex flex-wrap gap-3">
            {stabilizerSelected?.packs.map((item) => (
              ...
              >
                ...
                >
                  <Image
                    src={mappingStabilizerImage?.[item.value] || ""}
                    alt={item.name}
                    objectFit="contain"
                    fill
                    className={`scale-110`}
                ...
                <div className="text-left flex flex-col gap-2">
                  <div className="font-semibold text-sm">
                    {item.name}
                    {mappingStabilizerDetail?.[item.value] ? (
              ...
              >
                ...
                >
                  <Image
                    src={mappingStabilizerImage?.[item.value] || ""}
                    alt={item.name}
                    objectFit={item.value !== "2U" ? "contain" : "cover"}
                    fill
                ...
                <div className="text-left flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-4">
                    <SimpleTextField
                      ...
                      currentAmount={stabilizerSelected.totalWire}
                    />
                  </div>
                </div>
                <button
