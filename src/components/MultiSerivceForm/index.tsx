        ...
        >
          {TabInfo.map((tab, index) => (
            <Tab
              key={index}
              label={
                <div className="flex items-center gap-2" id={tab.id}>
                  {tab.icon} {tab.name}({mapTabLabel?.[tab.id] || 0})
                </div>
              }
      ...
      <CustomTabPanel value={activeTabIndex} index={0}>
        <div className="flex flex-col gap-3">
          <div className="flex justify-between items-center">
            ...
            >
              <span className="text-white flex items-center gap-2">
                {isKeyboardLimit ? (
                  <span className="text-gray-500">Đã đạt giới hạn</span>
                ) : (
                  <>
          ...
          {keyboardItems.map((item) => (
            <InputWrapperLegend key={item.id} className="mt-4 bg-white">
              <button
