...
const deliveryAddressAndFee = async ({
  ...
  const deliveryDistanceAndFeeResult = await calculateDeliveryDistanceAndFee(orderInfo.deliveryAddress)
...
async function getCoordinatesFromAddress(address: string): Promise<{ latitude: number; longitude: number } | null> {
  try {
...
async function calculateDeliveryDistanceAndFee(deliveryAddress: string): Promise<{ distance: number | null; extraFee: number }> {
...
function getRaffleTimeLeft(start: Date, end: Date, now: Date) {
  if (now < start) {
    const diff = start.getTime() - now.getTime()
    return { mode: EnumRaffleStatus.UPCOMING, diff }
  }
  if (now >= start && now <= end) {
    const diff = end.getTime() - now.getTime()
    return { mode: EnumRaffleStatus.ONGOING, diff }
  }
  return { mode: EnumRaffleStatus.ENDED, diff: 0 }
}
