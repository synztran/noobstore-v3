...
const CollectionPage = ({ title }: Props) => {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (isValid(resp)) {
        setCategories(resp.data as ICategory[])
      }
    })()
  }, [])
  ...
  return (
    <Base>
      <div className="mx-w-full p-6 sm:py-6 lg:px-8 relative z-1">
        ...
        <HeadSection title="Danh mục" setMobileFiltersOpen={setMobileFiltersOpen} />
        <article className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4 mt-4">
          <FilterSection isMobileOpen={mobileFiltersOpen} setMobileOpen={setMobileFiltersOpen} />
...
const HeadSection = ({ title = '', setMobileFiltersOpen }: { title: string; setMobileFiltersOpen: Dispatch<SetStateAction<boolean>> }) => {
  return (
    <div aria-labelledby="title" className="flex items-baseline justify-between border-b border-gray-200 pb-2">
      <h1 id="title" className="text-4xl font-bold tracking-tight text-gray-900 capitalize">
        {title}
      </h1>
