...
const HomePage = ({}) => {
  const { data: raffleFeaturedData, isPending: isLoadingFeaturedRaffle } = useRafflesQuery({
    ...
    <Base>
      <div className="flex flex-col gap-12 my-8">
        ...
        {/* <RaffleBlock
				...
				/> */}
        <RaffleBlockV2 raffleData={raffleFeaturedData?.data?.[0]} isLoading={isLoadingFeaturedRaffle} />
        <BannerTagPriceName />
        <CategoryWithTitleAndAction title="Bàn phím" items={[]} />
        <CommunitySharingPost title="Chia sẻ cộng đồng" posts={[]} onStartBuilding={() => {}} onSeeMore={() => {}} />
        <GuildAndTutorials />
        <GridLayoutBlock images={[]} />
        <StrongPoint />
        <HomeProductBlock title="Bàn phím" subTitle="Đa dạng layout từ 60% lên đến TKL" items={[]} id="keyboards" />

        <HomeProductBlock title="Switches" subTitle="Linear, clicky, tactile, kể cả silent" items={[]} id="switches" />
        {/* <HomeBanner />
