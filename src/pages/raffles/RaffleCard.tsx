import { Button } from "@/components/ReUIComponent/Button";
import { NEW_MISSING_IMAGE } from "@/constants/Images";
import { ChevronRight, Timer } from "lucide-react";
import Image from "next/image";
import Link from "next/link";

interface RaffleCardProps {
	raffle: {
		id: number;
		title: string;
		artist: string;
		image: string;
		timeLeft: string;
		price: number;
		featured?: boolean;
		aspect?: string;
		status?: string;
		isNew?: boolean;
	};
	isFeatured?: boolean;
}

export const RaffleCard = ({ raffle, isFeatured }: RaffleCardProps) => {
	if (isFeatured) {
		return (
			<Link href={`/raffles/${raffle.id}`}>
				<div className="group relative bg-white rounded-xl overflow-hidden border border-gray-200 hover:border-blue-500/40 transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/10 cursor-pointer">
					{/* Featured Ribbon */}
					<div className="absolute -right-12 top-6 rotate-45 bg-linear-to-r from-yellow-400 to-orange-400 py-1 w-40 text-center z-20 shadow-lg">
						<span className="text-[10px] font-bold text-black uppercase tracking-widest">
							Nổi Bật
						</span>
					</div>

					<div className="flex h-48">
						<div className="w-1/2 overflow-hidden relative">
							<Image
								src={raffle.image || NEW_MISSING_IMAGE}
								alt={raffle.title}
								fill
								className="object-cover transform group-hover:scale-110 transition-transform duration-700"
							/>
							<div className="absolute inset-0 bg-linear-to-r from-transparent to-white"></div>
						</div>

						<div className="w-1/2 p-6 flex flex-col justify-center">
							<div className="mb-1 text-xs font-bold text-blue-600 uppercase tracking-wide">
								{raffle.artist}
							</div>
							<h3 className="text-xl font-bold text-gray-900 mb-2 leading-tight">
								{raffle.title}
							</h3>
							<div className="flex items-center gap-2 text-gray-600 text-sm mb-4">
								<Timer size={16} /> Còn {raffle.timeLeft}
							</div>
							<div className="flex items-center justify-between mt-auto">
								<span className="text-gray-900 font-bold text-lg">
									${raffle.price.toFixed(2)}
								</span>
								<button className="w-8 h-8 rounded-full bg-blue-500/10 hover:bg-blue-500 text-blue-600 hover:text-white flex items-center justify-center transition-all">
									<ChevronRight size={16} />
								</button>
							</div>
						</div>
					</div>
				</div>
			</Link>
		);
	}

	return (
		<Link href={`/raffles/${raffle.id}`}>
			<div className="group relative bg-white rounded-xl overflow-hidden shadow-lg border border-gray-200 transition-all duration-300 hover:shadow-blue-500/20 hover:-translate-y-1 cursor-pointer">
				<div
					className={`relative overflow-hidden ${
						"aspect" in raffle && raffle.aspect === "tall"
							? "aspect-4/5"
							: "aspect" in raffle && raffle.aspect === "wide"
								? "aspect-4/3"
								: "aspect-square"
					}`}>
					<Image
						src={raffle.image || NEW_MISSING_IMAGE}
						alt={raffle.title}
						fill
						className="object-cover transition-transform duration-500 group-hover:scale-105"
					/>

					{"isNew" in raffle && raffle.isNew && (
						<div className="absolute top-3 left-3 bg-blue-500 text-white px-2 py-1 rounded-md text-xs font-bold shadow-lg">
							Mới
						</div>
					)}
					{"status" in raffle && raffle.status && (
						<div className="absolute top-3 right-3 bg-red-500 text-white px-2 py-1 rounded-md text-xs font-bold shadow-lg">
							{raffle.status}
						</div>
					)}
				</div>

				<div className="p-5">
					<div className="flex justify-between items-start mb-2">
						<div>
							<h3 className="text-lg font-bold text-gray-900 group-hover:text-blue-600 transition-colors">
								{raffle.title}
							</h3>
							<p className="text-sm text-gray-600">
								{raffle.artist}
							</p>
						</div>
						<div className="bg-blue-500/10 text-blue-600 px-2 py-1 rounded text-xs font-bold">
							${raffle.price.toFixed(2)}
						</div>
					</div>

					<div className="mt-4 pt-4 border-t border-gray-200 flex items-center justify-between">
						<div className="flex items-center gap-1 text-gray-600 text-xs font-semibold">
							<Timer size={14} /> {raffle.timeLeft}
						</div>
						<Button
							variant="outline"
							size="sm"
							className="text-xs font-bold uppercase tracking-wider">
							Join Raffle
						</Button>
					</div>
				</div>
			</div>
		</Link>
	);
};
