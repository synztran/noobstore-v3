...
const LoginPage = (): React.ReactElement => {
	...
	const SigupSchema = Yup.object().shape({
		firstName: Yup.string()
			.min(2, "Tên hơi ngắn ây ạ")
			.max(50, "Tên dài quá rồi ạ")
			.required("Bạn chưa nhập tên ạ"),
		lastName: Yup.string()
			.min(2, "Họ hơi ngắn ấy ạ")
			.max(50, "Họ dài quá rồi ạ")
			.required("Bạn chưa nhập họ ạ"),
		email: Yup.string()
		...
		passwordConfirm: Yup.string()
			.oneOf([Yup.ref("password")], "Mật khẩu không khớp ạ")
			.required("Bạn chưa nhập lại mật khẩu ạ"),
	});
					...
					{({ errors, touched }) => (
						<Form className="mx-auto flex flex-col gap-4">
							...
							<div className="flex gap-4">
								<button
									className="max-w-max px-4 py-2 rounded-sm bg-black text-white"
									onClick={() =>
										router.push(PAGE_LINK.LOGIN)
									}>
									Đăng nhập ngay
								</button>
								<button
									className="max-w-max px-4 py-2 rounded-sm border border-black"
									type="submit">
									Đăng ký
								</button>
							</div>
