...
export type ChatMessageType = 'text' | 'order'

export interface IChatOrderInfo {
  orderId: string
  totalPrice: number
...
type WebSocketMessage = {
  ...
  payload: {
    type?: 'ping' | 'pong' | 'message' | 'message_list' | 'message_list_request' | 'message_sync'
...
class WebSocketService {
  ...
  public initialize(user: IAuthUser) {
    ...
    if (typeof window !== 'undefined') {
      ...
      console.log(`[WebSocket] Initializing with roomId: ${this.roomId}, customerId: ${this.customerId}`)
  ...
  private connect() {
    ...
    console.log(`[WebSocket] Attempting to connect to server with roomId: ${this.roomId}`)
    ...
    try {
      ...
      this.ws.onopen = () => {
        console.log(`[WebSocket] Connected successfully to ${apiUrl}/${clientID}`)
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          this.messageCount++
          console.log(`[WebSocket] Received message (${this.messageCount}):`, message)

          // Handle sync messages
          if (message.action === 'sync_request' && message.payload.roomId !== this.roomId) {
            // Another tab is requesting sync
            this.send({
              action: 'sync_response',
              payload: {
          ...
          } else if (message.action === 'sync_response' && (message.payload.requestingRoomId === this.roomId || message.payload.requestingCustomerId === this.customerId)) {
            console.log(`[WebSocket] Received sync response from room: ${message.payload.respondingRoomId}`)
          }
  ...
  public send(message: WebSocketMessage) {
    if (this.ws?.readyState === WebSocket.OPEN) {
      this.messageCount++
      console.log(`[WebSocket] Sending message (${this.messageCount}):`, message)
