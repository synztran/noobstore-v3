...
import { EnumBackendServiceStepStatus, EnumBackendServiceType } from '@/interface/Client/Service'
import {
  ENUM_FILM_TYPE,
  ENUM_GREASE_TYPE,
  ENUM_SPRING_TYPE,
...
import { ArchiveBoxIcon, ArrowPathIcon, ChartPieIcon, ChatBubbleBottomCenterTextIcon, CursorArrowRaysIcon, FingerPrintIcon, FolderPlusIcon, UserGroupIcon, WrenchIcon } from '@heroicons/react/24/outline'
import { CircularProgress } from '@mui/material'
import { CircleCheck, CircleX, DoorOpen, ShieldCheck } from 'lucide-react'
import { COD_ICON, FLAGS_VIETNAM, GIF_PAYMENT_CHECKING, GIF_PAYMENT_PAID_SUCCESS, GIF_RAFFLE_LOADING, I3D_CREDIT_CARD, MOMO_LOGO, MOMO_VERTICAL_LOGO, PAYPAL_ICON, VIETINBANK_LOGO } from './Images'
...
export const CountryFlag = [{ id: 0, name: 'Việt Nam', imageUrl: FLAGS_VIETNAM, value: 'vietnam' }]
...
export const serviceFormText: Record<EnumServiceType, any> = {
  [EnumServiceType.KEYBOARD]: {
    ...
    subTitle: 'Dịch vụ assembled cho kit phím hoàn chỉnh, bao gồm thông tin phím, switch, stabilizer và dịch vụ đi kèm',
  },
  [EnumServiceType.SWITCHES]: {
    title: 'Đăng ký dịch vụ switch',
    subTitle: 'Thêm các thông tin về cái dịch vụ bạn cần sử dụng và tải lên những hình ảnh hiện tại của switch',
  },
  [EnumServiceType.STABILIZER]: {
    title: 'Đăng ký dịch vụ stabilizer',
    subTitle: 'Thêm các thông tin về cái dịch vụ bạn cần sử dụng và tải lên những hình ảnh hiện tại của stabilizer',
  },
  [EnumServiceType.OTHER]: {
    title: 'Thông tin dịch vụ khác',
    subTitle: 'Thêm các thông tin về cái dịch vụ bạn cần sử dụng và tải lên những hình ảnh hiện tại của phím',
  },
}
...
> = {
  [EnumSaleStatus.GB]: { label: 'Group buy', bgColor: 'bg-blue-500' },
  [EnumSaleStatus.INSTOCK]: { label: 'Sẵn hàng', bgColor: 'bg-green-500' },
  [EnumSaleStatus.OUTSTOCK]: { label: 'Hết hàng', bgColor: 'bg-red-50', color: 'text-red-600' },
  [EnumSaleStatus.TBD]: { label: '', bgColor: '' },
  [EnumSaleStatus.ALL]: { label: '', bgColor: '' },
}
...
export const UploadStatusProperty: Record<EnumUploadStatus, { color: string; text: string; icon?: React.ReactElement }> = {
...
export const mapPostSaleType: Record<EnumPostPriceType, { label: string; value: string }> = {
...
export const postProductAddonService = [
  ...
  },
  {
    ...
    price: 0.5,
    minPrice: 20000,
    subName: 'Hỗ trợ làm trung gian giao dịch, đảm bảo an toàn cho cả 2 bên mua và bán',
  },
]

export const RProductPartType: Record<EnumProductType, { label: string; value: EnumProductType; disabled?: boolean }> = {
...
export const ORDER_STATUS_LABEL: Record<EnumOrderStatus, { label: string; color: string }> = {
...
> = {
  ...
  [EnumPaymentStatus.REFUNDED]: {
    ...
    color: 'text-white',
    bgColor: 'bg-blue-400',
    subLabel: 'Chúng tôi đã hoàn lại thanh toán cho dịch vụ này. Vui lòng kiểm tra tài khoản của bạn.',
  },
  [EnumPaymentStatus.SUBMITTED]: {
    ...
    iconBadge: <CircularProgress size={16} sx={{ color: 'rgb(37 99 235)' }} />,
...
export const MAPPING_ICON_SUMMARY_SERVICE: Record<string, { icon: React.ReactNode; color: string }> = {
  INSURANCE_PACKAGE: {
...
export const mappingServiceName: Record<'keyboard' | 'switch' | 'stabilizer', { [x: string]: string }> = {
  keyboard: {
    solder: 'Hàn switch',
    desolder: 'Rã switch',
    clean: 'Vệ sinh phím',
...
export const mappingShippingMethodLabel: Record<EnumShippingMethodCode, string> = {
  [EnumShippingMethodCode.SELF_DELIVERY_SELF_PICKUP]: 'Khách tự gửi và nhận hàng',
  [EnumShippingMethodCode.STORE_DELIVERY_SELF_PICKUP]: 'Khách gửi hàng, shop giao hàng',
  [EnumShippingMethodCode.STORE_DELIVERY_STORE_PICKUP]: 'Shop lấy và giao hàng',
  [EnumShippingMethodCode.STORE_PICKUP_SELF_DELIVERY]: 'Shop lấy hàng, khách đến nhận hàng',
}
...
export const mappingLabelPaymentForm: Record<EnumPaymentForm, { label: string; icon?: string }> = {
...
export const mappingBookingServiceStatus: Record<EnumBackendServiceStepStatus, string> = {
...
export const mappingRaffleTimelineStatus: Record<EnumRafflePaymentStepStatus, string> = {
...
> = {
  ...
  [EnumRafflePaymentStatus.CANCELLED]: {
    label: 'Đã hủy',
    color: 'text-black',
    bgColor: 'bg-gray-400',
  },
  [EnumRafflePaymentStatus.REFUNDED]: {
    label: 'Đã hoàn tiền',
    color: 'text-blue-600',
    bgColor: 'bg-blue-100',
  },
...
export const mappingBankInfo: Record<
  string,
  {
    logo: string
  }
...
export const mapLabelEnumStabilizerMountType: Record<EnumStabilizerMountType, string> = {
