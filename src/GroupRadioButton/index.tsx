...
import { CountryFlag, VNCity } from "@/constants";
...
export default function GroupRadioButton(props: Props) {
	...
	return (
		<FormControl error={error} variant="standard">
			...
			{subTitle ? (
				<FormLabel className={styles.groupSubtitle}>
					{subTitle}
				</FormLabel>
			) : null}
			{options?.length === 1 ? (
				...
				<RadioGroup
					...
					className={`${isBorder ? "border border-gray-500 rounded-md" : ""} ${
						styles.mainGroup
					}`}>
					{options?.map((child) => {
						if (child?.textRight || child?.imagesRight) {
							return (
								<div key={child?.value as string}>
									...
									{name === "billingAddress" &&
									child?.isHaveCollapse &&
									child?.collapseType ===
										ENUM_CHECKOUT_COLLAPSE_TYPE.NEW_ADDRESS ? (
										<CollapseNewBillingAddress
				...
				<div className="flex gap-4">
					...
					<div className="flex flex-col w-1/2">
						...
						<TextField
							...
							type="text"
							placeholder="Tên"
							className="h-12 rounded-sm border border-gray-400"
							variant="outlined"
							InputProps={{
								className: styles.collapseInput,
							}}
